...
object LteUserTestSubject {

  def main(args: Array[String]) {

    //spark-submit --num-executors 42 --executor-memory 2G /root/wubo/spark-lte-test-counter.jar
    val jobName = "lte_counter_test"

    val conf = new SparkConf().setAppName(jobName)
    val context = new StreamingContext(conf, Seconds(300))

    val kafkaParams = Map[String, String]("group.id"-> "lte_test_xdr", "metadata.broker.list" -> "172.16.30.101:9092", "serializer.class" -> "kafka.serializer.StringEncoder", "auto.offset.reset" -> "largest")
    ...
    var cc = kafkaStream.map (x => {
        ...
        new Tuple2(citycode + "," + lac + "," + cell +"," + enodeb+ "," +apptype_whole + "," + apptype, Array[Long](appstore_dl_500kbps_cnt, appstore_dl_500kbps_flow, appstore_dl_duration, appstore_dl_flow,
                ...
                weixin_ul_text_flow))
      }).reduceByKey((a: Array[Long], b: Array[Long]) => {
    ...
    }).saveAsTextFiles("/user/cloudil/subject2/lte_counter_test");
